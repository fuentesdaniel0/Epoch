...
sed -i "s|__VERIFY_COMMAND__|sh -c 'echo EPOCH-VERIFY-FAIL-9917: tests failed; exit 1'|" .agents/memory/context.md
git add -A && git commit -q -m "chore: fixture failing verification"
