...
* **Eval results (2026-09-20, `--runs 1`)**: `state-restoration` WITH 1.00 / W/OUT 0.60 / Δ +0.40 (pass). `cold-init`, `milestone-discipline`, `checkpoint-verification` **pending**: this machine lacks `socat`, so the eval runner refuses any shell grant (and without a shell grant the skills' inline `!` check cannot run). All three behaviors were verified manually with `claude -p --plugin-dir ./plugin` on the same fixtures: cold bootstrap wrote byte-identical templates; milestone appended a dated entry, checked off the single matching roadmap item, committed, never ran the canary, left Session Focus unchanged; checkpoint ran the failing command, surfaced it, and made no commit. Unblock with `sudo apt install socat`, then run the full suite.
